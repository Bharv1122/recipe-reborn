...
export default function ResetPasswordScreen() {
  ...
  return <Screen><Stack.Screen options={{ headerShown: true, title: 'Choose new password', headerTintColor: colors.green }} /><ScrollView contentContainerStyle={styles.content}><Card><Text style={styles.title}>Choose a new password</Text><Field accessibilityLabel="New password" autoComplete="new-password" value={password} onChangeText={setPassword} secureTextEntry placeholder="At least 8 characters" /><Field accessibilityLabel="Confirm new password" autoComplete="new-password" value={confirm} onChangeText={setConfirm} secureTextEntry placeholder="Confirm password" /><Button label="Update password" onPress={submit} loading={busy} disabled={password.length < 8 || !confirm} />{message ? <><Text accessibilityLiveRegion="polite" style={styles.success}>{message}</Text><Button label="Sign in" onPress={() => router.replace('/(auth)/sign-in')} /></> : null}<InlineError message={error} /></Card></ScrollView></Screen>;
}
