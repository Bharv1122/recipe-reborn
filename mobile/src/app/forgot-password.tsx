...
export default function ForgotPasswordScreen() {
  ...
  return <Screen><Stack.Screen options={{ headerShown: true, title: 'Reset password', headerTintColor: colors.green }} /><ScrollView contentContainerStyle={styles.content}><Card><Text style={styles.title}>Reset your password</Text><Text style={styles.body}>We’ll email the same secure reset link used by RecipeReborn.com.</Text><Field accessibilityLabel="Email address" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" autoComplete="email" placeholder="Email address" /><Button label="Email reset link" onPress={submit} loading={busy} disabled={!email.includes('@')} />{message ? <Text accessibilityLiveRegion="polite" style={styles.success}>{message}</Text> : null}<InlineError message={error} /><Button label="Back to sign in" secondary onPress={() => router.replace('/(auth)/sign-in')} /></Card></ScrollView></Screen>;
}
