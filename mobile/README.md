...
- Shopping: SQLite caches full lists. After one online sign-in/open with this version, a minimal identity in Keychain/Keystore lets cached lists open after an offline restart. This identity never grants a subscription or server permission. Check/uncheck works offline; the newest queued value is preserved during an in-flight sync. Reopen or pull to refresh Shopping after reconnecting to sync. List and item creation require a connection.
