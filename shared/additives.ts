// Shared ingredient-name matcher for the web and native apps.
...
export function detectAdditives(text: string | null | undefined): DetectedAdditive[] {
  ...
  for (const a of ADDITIVES) {
    ...
    const spans = a.match.flatMap((phrase) => {
      const matches: { start: number; end: number }[] = [];
      let start = haystack.indexOf(phrase);
      while (start !== -1) {
        const end = start + phrase.length;
        const isEmbeddedAbbreviation = phrase.length <= 4 &&
          (/[a-z]/.test(haystack[start - 1] || '') || /[a-z]/.test(haystack[end] || ''));
        if (!isEmbeddedAbbreviation && !matchedSpans.some((span) => start >= span.start && end <= span.end)) matches.push({ start, end });
        start = haystack.indexOf(phrase, end);
      }
      return matches;
    });
    if (spans.length) {
